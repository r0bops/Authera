...
| Rich conditions, evaluated by code | “If it drops below USD 150” is the per-purchase limit plus the price watcher; “up to 3 times” is `maxFulfillments`; the date window (`INTENT_DATES`, ± `dateFlexibilityDays`) **stopovers and total travel time** (`maxStops`, `maxDurationMinutes` — “direct only”, “under 8 hours”; checks `INTENT_STOPS`, `INTENT_DURATION`; an itinerary whose connections are unknown can never claim to be direct), a **departure-time window** (`departureTimeFrom/To`, local time at the origin — “mornings only”, “after 6 pm” — check `INTENT_TIME`, approvable on ask-plans) and an **approval ceiling** (`approvalCeilingMinor`) make escalation tiered on ask-plans: within the limit buys alone, up to the ceiling waits for the human, above the ceiling is blocked before anyone is asked. Block-plans hand a **near miss** to the human instead of a flat block: a progressive tolerance of 10 % on small budgets sliding to 7 % on large ones (`approvalTolerance`), recorded as the `APPROVAL_TOLERANCE` check. And the price watcher does what the brief says: when an offer inside the plan appears, the agent attempts it at once (`PRICE_WATCH_AUTO_BUY`, one attempt per offer) — the gateway still decides |
