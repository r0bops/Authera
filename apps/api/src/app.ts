...
} from "./routes/webhooks/webhooks.js";
...
export function createApp(deps: AppDependencies): App {
  ...
  app.use(
    ...
    bodyLimit({
      ...
      onError: (c) =>
        fail(c, 413, "PAYLOAD_TOO_LARGE", "Request body too large"),
    }),
  );
  ...
  app.onError((error, c) => {
    if (error instanceof ApiProblem) {
      if (error.status >= 500)
        c.get("logger").error({ err: error }, "api problem");
  ...
  app.route(
    ...
    healthRoutes({
      ...
      ...(deps.checkMigrations
        ? { checkMigrations: deps.checkMigrations }
        : {}),
    }),
  );
  ...
  if (deps.services) {
    ...
    if (
      deps.config.markets.priceWatchIntervalMs > 0 &&
      markets.length + goodsMarkets.length > 0
    ) {
      new PriceWatcher({
        checkout,
        listMandates: async () =>
    ...
    app.use(
      "/api/agent/*",
      agentSignature({ store: identity, clock, tag: AGENT_TAGS.browse }),
    );
    ...
    const ap2Evidence = new Ap2EvidenceService({
      evidence,
    ...
    app.route(
      "/api",
      humanEvidenceRoutes({ db, approvals, disputes, evidence, ap2Evidence }),
    );
    ...
    app.route(
      "/webhooks",
      providerWebhookRoutes({ processor: paymentProcessor, payments }),
    );
    if (
      deps.config.demo.enabled &&
      paymentProcessor instanceof MockPaymentProcessor
    ) {
      app.use("/webhooks/mock/*", sessionMiddleware(sessionDeps));
      app.use(
        "/webhooks/mock/*",
        csrfGuard({ publicBaseUrl: deps.config.publicBaseUrl }),
      );
      app.route(
        "/webhooks",
        mockWebhookRoutes({ processor: paymentProcessor, payments }),
      );
    }
  }
